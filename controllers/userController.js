...
async function users_creation(req, res) {
    ...
    let user = new User();
    user.creation(headers.login, headers.password, headers.email, headers.role)
        .then((result)=>{
            user.find({id: result})
...
async function user_edit(req, res) {
    try {
        const user_id = req.params.user_id;
        const new_data = JSON.parse(req.headers.data);
        let user = new User();
        user.find({id: user_id}).then((result)=>{
            if (result.length === 0)
                res.json(new Response(false, 'Похоже пользователя с таким id не существует'));
            else {
                new_data.id = user_id;
                user.updateById(new_data).then((result)=>{
